...
> Used by `/onboard-research` Phase 2 to recommend connectors per business type. Schema mirrors `cowork-ai-os` v0.10 `browse-connectors` exactly. Last verified: 2026-05-14.
...
- **first-week test:** Ask Cowork *"using perplexity, what's the latest on [user's competitor]?"*
...
- **add it:** Already installed as a Cowork skill. Verify with `/firecrawl-scrape https://example.com`.
- **what you unlock:** Pulling clean markdown from any URL into your research workflow
- **first-week test:** `/firecrawl-scrape <competitor-pricing-page>`
...
- **first-week test:** Run `/meeting-recap <fathom-url>` on yesterday's most important call
...
- **add it:** Already installed as MCP. Verify with `mcp__plugin_context7__query-docs`.
...
- **first-week test:** `vidiq_outliers` against a competitor channel
...
- **add it:** Already installed as MCP plugin. Verify with `mcp__plugin_playwright__browser_navigate`.
...
- **first-week test:** Run a brief and confirm it lands in the chosen Notion database
...
- **first-week test:** `/research-brief` on a topic that has 3+ existing Drive docs; verify they appear in sources
